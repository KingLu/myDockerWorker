...
# 如果使用自托管的 GitLab，需要设置 GitLab 的 URL
c.GitLabOAuthenticator.gitlab_url = os.getenv('GITLAB_URL')

# 设置允许访问 JupyterHub 的用户白名单
import os
# 从环境变量获取白名单用户列表，使用逗号分隔用户名
whitelist = os.getenv('JUPYTERHUB_WHITELIST')
if whitelist:
    c.Authenticator.whitelist = set(whitelist.split(','))

# 设置管理员用户
# 列在这里的用户将拥有 JupyterHub 的管理员权限
# 从环境变量获取管理员用户列表，使用逗号分隔用户名
